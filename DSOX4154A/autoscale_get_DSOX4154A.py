...
class DSO_X4154A():
        def __init__(self,channel=None,filename=None,host=None,query=None,command=None,typ="BYTE"):
            ...
            self.scope.write(':WAVeform:POINTS: 8000000')
            ...
            self.scope.write(':TRIGger:EDGE:SOURce LINE')
            ...
            if filename:
                self.rescale222()                                         #RESCALES
                self.ask_OPC()                                           #OPERATION COMPLETE?
                self.single()                                           #SINGLE TRACE THEN STOPS
                self.ask_OPC()                                           #OPERATION COMPLETE?
  ...
  #%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5    
        ...
        def mode_of_trigger(self):
            q_result = self.scope.ask(':TRIGger:EDGE:SOURce?')
            print 'Trigger source: %s' %q_result
            return q_result
        ...
        def rescale222(self):  
            Vpp = float(self.scope.ask(':MEASURE:VPP? CHANNEL1')) # current signal span V peak to peak
            curr_scale =float(self.scope.ask(':CHANNEL1:SCALE?'))# get the vertical scale
            number = float(Vpp/curr_scale)
            if (number >= 6.30 and number <= 6.5) :## check if the signal is too high
                #self.scope.write(':AUToscale')
                #self.scope.write(':TRIGger:EDGE:SOURce LINE')
                print 'its ok1'
                time.sleep(1.0)
            elif (number <= 6.30 or number >= 6.5) :
                self.scope.write(':AUToscale')
                self.scope.write(':TRIGger:EDGE:SOURce LINE')
        ...
        def ask_OPC(self):    
            print self.scope.ask("*OPC?"), '[0=No   1=YES]' 
